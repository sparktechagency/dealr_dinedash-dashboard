...
import ReuseSearchInput from "../../UI/ReuseSearchInput";
...
const AllDealsTab = ({
  ...
  const { data, isFetching } = useAllApprovedDealQuery({
    page,
    limit,
    name: searchText,
    city: searchCityText,
  ...
  return (
    <div className="mt-3 h-screen">
      <div className="h-fit flex gap-2 items-center mb-16">
        <div>
          <Typography.Title
            level={5}
        ...
        <div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 items-baseline gap-6 w-full">
            {allDeals?.map((item, index) => {
              ...
              const todayHours = item.openingHours?.find(
                (h) => h.day === todayDay
              );
